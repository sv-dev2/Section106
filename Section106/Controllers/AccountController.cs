...
namespace Section106.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public AccountController(ApplicationUserManager userManager, ApplicationSignInManager signInManager, ICommonService commonService, IUserService userService)
        {
        ...
        public ActionResult Register()
        {
            ...
            registerVM.States = _commonService.GetStates();
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (model.OfficePhone != null)
                    model.OfficePhone = model.OfficePhone.Replace("(", "").Replace(")", "").Replace("-", "").Replace(" ", "");
                var user = new ApplicationUser { UserName = model.UserName.Trim(), Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    // Adding Entity
                    var entityResult = _userService.CreateUser(model, user.Id);
                    if (entityResult)
                    {
        ...
        [AllowAnonymous]
        public ActionResult CreateReviewerAccount()
        {
            var registerVM = new RegisterViewModel();
            var ReviewersRoleList = new List<RoleVM>();

            ReviewersRoleList.Add(new RoleVM() { RoleId = "Architectural", RoleName = "Architectural" });
            ReviewersRoleList.Add(new RoleVM() { RoleId = "Archaeology", RoleName = "Archaeology" });
            ReviewersRoleList.Add(new RoleVM() { RoleId = "Technical Assistance", RoleName = "Technical Assistance" });
            ReviewersRoleList.Add(new RoleVM() { RoleId = "Landmarks", RoleName = "Landmarks" });

            registerVM.ReviewersRoleList = ReviewersRoleList;
            registerVM.States = _commonService.GetStates();
            return View(registerVM);
        }
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterReviewer(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.UserName.Trim(), Email = model.Email };
                ...
                if (result.Succeeded)
                {
                    ...
                    if (entityResult)
                    {
                        return RedirectToAction("ReviewerAccounts", "Admin");
                    }
                }
                AddErrors(result);
            }
            ...
            // If we got this far, something failed, redisplay form
            return RedirectToAction("CreateReviewerAccount", "Account");
        }

        public ActionResult EditReviewerAccount(string UserId)
        {
            var contactVM = new ContactVM();
            contactVM = _userService.GetSubmittingContactByUserId(UserId);
            contactVM.States = _commonService.GetStates();
            return View(contactVM);
        }

        public ActionResult UpdateReviewerUser(ContactVM model)
        {
            if (ModelState.IsValid)
            {
            ...
            TempData["ErrorMessage"] = "Account not updated.";
            return RedirectToAction("EditReviewerAccount", "Account");
        }
